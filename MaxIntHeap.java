...
public class MaxIntHeap {
    ...
    // the way how we convert our binary tree into the array.
    ...
    private void ensureCapactity() {
        if (size == capactity) {
            items = Arrays.copyof(items, newLength: capactity * 2);
            capactity *= 2;
        }
    }
    ...
    public void insert(int item) {
        ensureCapactity();
        items[size] = item;  // put in last spt
        size++;
        heapityUp();
    }
